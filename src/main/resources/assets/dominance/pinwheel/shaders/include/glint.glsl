...
vec4 glint(vec4 _min, vec4 _max, float _GameTime, float _dist, float _shineSpeed, float _colorSpeed, float _distShine, float _cScale) {
...
vec4 glintEnchant(float _GameTime, float _dist) {
...
vec4 glintCube(vec4 _min, vec4 _max, float _GameTime, float _dist, vec2 _FragTexCoord) {
    return glint(_min, _max, _GameTime, _dist, 4, 2, 3, sqrt(clamp(distance(_FragTexCoord, vec2(0.5, 0.5)) / 0.7, 0, 1)));
}